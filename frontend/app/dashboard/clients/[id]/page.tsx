...
export default function ClientDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'policies' | 'family' | 'ledger'>('info');
  ...
  const fetchClient = async () => {
    try {
      const [clientRes, ledgerRes] = await Promise.all([
        clientAPI.getById(params.id as string),
        clientAPI.getLedger(params.id as string)
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className={`text-2xl font-bold ${(ledgerSummary?.balance || 0) > 0 ? 'text-red-600' : 'text-green-600'}`}>
      ...
      <div className="border-b">
        <div className="flex space-x-8">
          {(['info', 'policies', 'family', 'ledger'] as const).map((tab) => (
            <button
              key={tab}
      ...
      {activeTab === 'ledger' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="font-medium text-gray-900">Ledger / Khata</h3>
